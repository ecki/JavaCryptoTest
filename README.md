...
If you have a JKS file you can this class to extrace all certificate chains.
This is especially helpful if you have a KeyStore which does no longer open because of
certificates with illegal DER encoding (leading zeros) which are rejected since Java 8U121.

    java -cp target/classes net.eckenfels.test.certpath.KeystoreExploder c:\temp\bla.jks out

This will create the specified target directory and one `.crt` entry per Keystore Alias.
The files are PEM style certificate (chains) with some clear text headers.

## CertExplorer

On a directory containign *.crt|cer files (maybe generated by *KeystoreExploder*) you can run the
following command to check if the certificate parses and what the Subject DN is:

    java -cp target/classes net.eckenfels.test.certpath.CertExplorer out

Typically you will use older Java to explode the JKS and newer Java to valiate the single certs.