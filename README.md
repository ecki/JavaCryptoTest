...
If you have a JKS file you can use this class to extract all certificate(chain)s.
This is especially helpful if you have a KeyStore which does no longer open because of
certificates with illegal DER encoding (leading zeros) which are rejected with Java 8u121+8u131.

    java -cp target/classes net.eckenfels.test.certpath.KeystoreExploder c:\temp\broken.jks certdir

This will create the specified target directory `certdir` and one `.crt` entry per Keystore Alias.
Those files are PEM-style certificate (chains) with some clear text headers.

## CertExplorer

On a directory containign `*.crt` or `*.cer` files (maybe generated by *KeystoreExploder*) you can run the
following command to check if the certificate parses and what the Subject DN is:

    java -cp target/classes net.eckenfels.test.certpath.CertExplorer certdir

Typically you will use older Java to explode the JKS and newer Java to valiate the extracted cert files.
For each problematic certification file the name and the exception is printed:

    loading C:\temp\certdir\alias1.crt  CN=Test, C=US
    loading C:\temp\certdir\alias2.crt  ???java.security.cert.CertificateParsingException:
	  java.io.IOException: subject key, java.security.InvalidKeyException: Invalid RSA public key
